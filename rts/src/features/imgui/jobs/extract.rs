...
pub struct ImGuiExtractJob<'extract> {
    imgui_manager: TrustCell<ResourceRefBorrowMut<'extract, ImguiManager>>,
...
impl<'extract> ImGuiExtractJob<'extract> {
    pub fn new(
        ...
        Arc::new(ExtractJob::new(
            Self {
                imgui_manager: TrustCell::new(
                    extract_context
                        .extract_resources
                        .fetch_mut::<ImguiManager>(),
                ),
...
impl<'extract> ExtractJobEntryPoints<'extract> for ImGuiExtractJob<'extract> {
    fn begin_per_frame_extract(&self, context: &ExtractPerFrameContext<'extract, '_, Self>) {
        let imgui_manager_mut = &mut self.imgui_manager.borrow_mut();
        let imgui_draw_data = imgui_manager_mut.copy_draw_data();
        let view_ubo = {
