...
use rafx_plugins::{
    ...
    shaders::{
        ...
        mesh_adv::mesh_adv_textured_frag::PerViewDataUniform as MeshPerViewFragmentShaderParam,
    },
};
...
const PER_VIEW_DESCRIPTOR_SET_INDEX: u32 =
    rafx_plugins::shaders::mesh_adv::mesh_adv_textured_frag::PER_VIEW_DATA_DESCRIPTOR_SET_INDEX
        as u32;
const PER_MATERIAL_DESCRIPTOR_SET_INDEX: u32 =
    rafx_plugins::shaders::mesh_adv::mesh_adv_textured_frag::PER_MATERIAL_DATA_DESCRIPTOR_SET_INDEX
        as u32;
...
pub struct DynMeshPrepareJob<'prepare> {
...
impl<'prepare> DynMeshPrepareJob<'prepare> {
    pub fn new(
        ...
        for view in frame_packet.view_packets() {
            if view
                .view()
                .feature_flag_is_relevant::<DynMeshUntexturedRenderFeatureFlag>()
            {
...
impl<'prepare> PrepareJobEntryPoints<'prepare> for DynMeshPrepareJob<'prepare> {
    fn begin_per_frame_prepare(&self, context: &PreparePerFrameContext<'prepare, '_, Self>) {
        let mut per_frame_submit_data = Box::new(DynMeshPerFrameSubmitData {
            num_shadow_map_2d: 0,
    ...
    fn prepare_render_object_instance(
        ...
        context.set_render_object_instance_submit_data(DynMeshRenderObjectInstanceSubmitData {
            model_matrix_offset,
        });
    }
    ...
    fn prepare_render_object_instance_per_view(
        ...
        if let Some(extracted_data) = context.render_object_instance_data() {
            ...
            for (mesh_part_index, mesh_part) in
                extracted_data.dyn_mesh.inner.mesh_parts.iter().enumerate()
            {
                ...
                if view.phase_is_relevant::<WireframeRenderPhase>()
                    && view.feature_flag_is_relevant::<DynMeshWireframeRenderFeatureFlag>()
                {
                    let material_pass_resource = mesh_part
    ...
    fn end_per_view_prepare(&self, context: &PreparePerViewContext<'prepare, '_, Self>) {
        ...
        let view = context.view();
        let is_lit = !view.feature_flag_is_relevant::<DynMeshUnlitRenderFeatureFlag>();
        let has_shadows = !view.feature_flag_is_relevant::<DynMeshNoShadowsRenderFeatureFlag>();
            ...
            let opaque_per_view_descriptor_set_layout = {
                ...
                for id in 0..context.render_object_instances().len() {
                    ...
                    if let Some(extracted_data) = context.render_object_instances_data().get(id) {
                        let mesh_parts = &extracted_data.dyn_mesh.inner.mesh_parts;
                ...
                |per_view_descriptor_set_layout| {
                    descriptor_set_allocator
                        .create_descriptor_set(
                            &per_view_descriptor_set_layout,
                            rafx_plugins::shaders::mesh_adv::mesh_adv_textured_frag::DescriptorSet0Args {
                                shadow_map_images,
    ...
    fn end_per_frame_prepare(&self, context: &PreparePerFrameContext<'prepare, '_, Self>) {
        ...
        *model_matrix_buffer = if self.render_object_instance_transforms.len() > 0 {
            ...
            let vertex_buffer_size = self.render_object_instance_transforms.len() as u64
                * std::mem::size_of::<DynMeshModelMatrix>() as u64;
    ...
    type SubmitPacketDataT = DynMeshRenderFeatureTypes;
}
