use crate::assets::terrain::{
    TerrainConfigAssetData, TerrainConfigAssetSourceData, TerrainMaterialConfig,
};
use distill::{
    core::AssetUuid,
...
pub struct TerrainConfigImporterStateStable {
    terrain_asset_uuid: Option<AssetUuid>,
    material_instance_asset_uuids: Vec<AssetUuid>,
}

impl From<TerrainConfigImporterStateUnstable> for TerrainConfigImporterStateStable {
    fn from(other: TerrainConfigImporterStateUnstable) -> Self {
        let mut stable = TerrainConfigImporterStateStable::default();
        stable.terrain_asset_uuid = other.terrain_asset_uuid.clone();
        stable.material_instance_asset_uuids = other.material_instance_asset_uuids.clone();
        stable
    }
}

#[derive(Default)]
pub struct TerrainConfigImporterStateUnstable {
    terrain_asset_uuid: Option<AssetUuid>,
    material_instance_asset_uuids: Vec<AssetUuid>,
}

impl From<TerrainConfigImporterStateStable> for TerrainConfigImporterStateUnstable {
    fn from(other: TerrainConfigImporterStateStable) -> Self {
        let mut unstable = TerrainConfigImporterStateUnstable::default();
        unstable.terrain_asset_uuid = other.terrain_asset_uuid.clone();
        unstable.material_instance_asset_uuids = other.material_instance_asset_uuids.clone();
        unstable
    }
}
...
pub struct TerrainConfigImporter;
impl Importer for TerrainConfigImporter {
    ...
    type Options = ();
    type State = TerrainConfigImporterStateStable;
    ...
    fn import(
        ...
        let mut unstable_state: TerrainConfigImporterStateUnstable = stable_state.clone().into();
        ...
        let source_data = ron::de::from_reader::<_, TerrainConfigAssetSourceData>(source)?;
        ...
        for (idx, source) in source_data.materials.into_iter().enumerate() {
            ...
            materials.push(TerrainMaterialConfig {
                source,
